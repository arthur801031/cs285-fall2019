...
class DQNAgent(object):
    ...
    def step_env(self):
        ...
        # DoneTODO store the latest observation into the replay buffer
        # HINT: see replay buffer's function store_frame
        self.replay_buffer_idx = self.replay_buffer.store_frame(self.last_obs)

        eps = self.exploration.value(self.t)
        # DoneTODO use epsilon greedy exploration when selecting action
        # any action with probability eps; otherwise, maxQt(a)
        ...
        if perform_random_action:
            ...
            # DoneTODO query the policy to select action
            ...
            # DoneTODO query the policy with enc_last_obs to select action
            action = self.actor.get_action(enc_last_obs)

        # DoneTODO take a step in the environment using the action from the policy
        # HINT1: remember that self.last_obs must always point to the newest/latest observation
        # HINT2: remember the following useful function that you've seen before:
            #obs, reward, done, info = env.step(action)
        obs, reward, done, info = self.env.step(action)

        # DoneTODO store the result of taking this action into the replay buffer
        # HINT1: see replay buffer's store_effect function
        # HINT2: one of the arguments you'll need to pass in is self.replay_buffer_idx from above
        self.replay_buffer.store_effect(self.replay_buffer_idx, action, reward, done)

        # DoneTODO if taking this step resulted in done, reset the env (and the latest observation), otherwise set last obs to obs
        if done:
            self.last_obs = self.env.reset()
        else:
            self.last_obs = obs
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts and \
            ...
            # DoneTODO populate the parameters and implement critic.update()
            loss = self.critic.update(ob_no, ac_na, re_n, next_ob_no, terminal_n)

            # DoneTODO: load newest parameters into the target network
