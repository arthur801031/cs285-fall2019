...
class MPCPolicy:
    ...
    def sample_action_sequences(self, num_sequences, horizon):
        # DoneTODO(Q1) uniformly sample trajectories and return an array of
    ...
    def get_action(self, obs):
        ...
        for model in self.dyn_models:
            ...
            sim_obs = np.tile(obs, (self.N, 1)) # (1000, 4)
            model_rewards = np.zeros(self.N) # (1000,)
        ...
        predicted_rewards = np.mean(predicted_rewards_per_ens, axis = 0) # DoneTODO(Q2)

        # pick the action sequence and return the 1st element of that sequence
        best_index = np.argmax(predicted_rewards) #DoneTODO(Q2)
        best_action_sequence = candidate_action_sequences[best_index] #DoneTODO(Q2)
        action_to_take = best_action_sequence[0] # DoneTODO(Q2)
        return action_to_take # the None is for matching expected dimensions
